...
export function getSinglePost(fullPath, path, fields = []) {
  // Create a browser friendly slug.
...
export async function getHtml(markdown) {
  ...
  const processMarkup = await remark().use(html).process(markdown)
 ...
 * @param {boolean} useSystemPath  Whether or not to return a full system path.
 * @returns string
 */
export function getPath(slug, useSystemPath = false) {
  // First, clean up the incoming array.
  const cleanSlug = slug.toString().replace(/,/g, '/').replace(/\.md$/, '')

  // Then, append the file extension.
  const addFileExt = `${cleanSlug}.md`

  // If we need a system path...
  if (useSystemPath) {
    return join(systemPath, addFileExt)
  }

  return join(postsDirectory, addFileExt)
}
